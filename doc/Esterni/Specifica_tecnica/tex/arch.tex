...
Il diagramma delle classi è un normale diagramma UML che offre un sottoinsieme dei formalismi UML, ispirato al \emph{20 percent of UML that helps you do 80 percent of your work} di cui parla Martin Fowler in \emph{UML Distilled}.

Il diagramma che noi chiamiamo di attività è, in realtà, un diagramma ``a blocchi'', cioè una sequenza di elementi grafici che rappresentano operazioni atomiche (ad esempio un \texttt{return} o un'assegnazione) oppure raggruppano al loro interno un'altra sequenza di blocchi (ad esempio un blocco \texttt{while} che raggruppa una sequenza di blocchi).
