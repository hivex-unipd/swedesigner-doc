...
\item l'attore può scegliere di modificare il contenuto del blocco if (UC5.22.2);
\item l'attore può scegliere di modificare il contenuto del blocco else (UC5.22.3);
\item l'attore può scegliere di modificare il commento (UC5.22.4);
\item l'attore può confermare le modifiche apportate al blocco if/else (UC5.22.5).
\end{enumerate}
\end{itemize}	
%\item l'attore può modificare un blocco while (UC5.23);
\subsubsection{UC5.: Modifica }
\label{UC5.20}
\begin{itemize}
\item \textbf{Attori}: Utente;
\item \textbf{Descrizione}: l'attore può modificare ;
\item \textbf{Precondizione}: l'attore ha selezionato il blocco  e il sistema visualizza il form di inserimento dati ;	
\item \textbf{Postcondizione}: il sistema visualizza il blocco   modificato;	
\item \textbf{Scenario principale}:
L'attore modifica il blocco   selezionato.	
%\item l'attore può modificare un blocco for (UC5.24);
\subsubsection{UC5.: Modifica }
\label{UC5.20}
\begin{itemize}
