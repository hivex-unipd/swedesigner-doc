...
In particolare, è richiesto che l'utente dell'\gloss{editor} abbia un certo grado di familiarità con i seguenti concetti:
\begin{itemize}
	\item Le principali convenzioni della modellazione \gloss{UML} e, nello specifico, i \gloss{diagrammi delle classi} e i \gloss{diagrammi delle attività}.
...
Il prodotto sviluppato risulterà fruibile da qualsiasi piattaforma desktop con operante un \gloss{browser} web compatibile con le tecnologie \gloss{HTML5}, \gloss{CSS3} e \gloss{JavaScript}. In particolare, l'elenco dei \gloss{browser} il cui supporto è garantito è contenuto nella sezione \ref{sec:reqvincolo} relativa ai requisiti di vincolo.
...
\begin{itemize}
	\item \textbf{Attori}: Utente;
	\item \textbf{Descrizione}: l'attore visualizza un opportuno messaggio d'errore nel caso in cui tenti di inserire un file incompatibile rispetto all'\gloss{editor} in uso;
...
\begin{itemize}
	...
	\item \textbf{Precondizione}: il sistema avviato mostra la pagina iniziale dell'applicazione;
	\item \textbf{Postcondizione}: il sistema crea un progetto vuoto e presenta all'attore un \gloss{diagramma delle classi} vuoto con un menù;
	\item \textbf{Scenario principale}:
...
\begin{itemize}
	...
	\item \textbf{Postcondizione}: il sistema ha ricevuto tutti gli input dall'attore sulla classe che questo ha definito e mostra il \gloss{diagramma delle classi} (in cui, per ogni metodo, viene mostrato un riferimento ad un \gloss{diagramma delle attività} vuoto);
	\item \textbf{Scenario principale}:
	\begin{enumerate}
...
\begin{itemize}
	...
	\item \textbf{Scenario principale}: l'attore inserisce un nome per identificare la classe corrente univoco all'interno del \gloss{diagramma delle classi} che sta gestendo.
\end{itemize}
...
%		\item l'attore non ha inserito il tipo dell'attributo; % Ho inserito il tipo predefinito (Object) come scenario alt. in UC4.1.4.3
...
\begin{itemize}
	...
	\item \textbf{Precondizione}: il sistema visualizza il \gloss{diagramma delle classi}, con la classe correntemente gestita dall'attore;
...
\begin{itemize}
	\item \textbf{Attori}: Utente;
	\item \textbf{Descrizione}: l'attore realizza e gestisce un \gloss{diagramma delle attività} per ogni metodo presente nel \gloss{diagramma delle classi} in modo tale da darne una implementazione più in dettaglio. Essa viene effettuata mediante blocchi specifici per ogni costrutto, per la gestione delle variabili e dei metodi. Tale diagramma quindi non è comforme allo standard \gloss{UML};
...
\begin{itemize}
	...
	\item \textbf{Postcondizione}: il sistema ha ricevuto il dato richiesto;
	\item \textbf{Scenario principale}: l'attore deve inserire uno o più blocchi tra quelli resi disponibili dall'\gloss{editor}.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Scenario principale}: l'attore muove il blocco selezionato in una nuova posizione del \gloss{diagramma delle attività} correntemente gestito.
\end{itemize}
...
\begin{itemize}
	\item \textbf{Attori}: Utente;
	\item \textbf{Descrizione}: l'attore può ottenere (scaricare) automaticamente un file .jar contenente l'applicativo generato dai diagrammi da esso forniti e realizzati mediante l'\gloss{editor}, i diagrammi stessi ed il codice sorgente in linguaggio target;
	...
	\item \textbf{Postcondizione}: il sistema genera l'applicativo derivante dai due diagrammi, i diagrammi stessi ed il codice sorgente;
	\item \textbf{Scenario principale}: l'attore scarica il file .jar contenente tre file quali l'applicativo, i diagrammi da lui realizzati ed il codice sorgente;
