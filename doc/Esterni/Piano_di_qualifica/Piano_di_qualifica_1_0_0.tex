	...
	\\Le fasi in cui esso si articola sono quattro:
		...
		In questa sezione vengono elencati i principali processi software identificati dal team sulla base dello standard ISO/IEC 12207:2008 e per ognuno di essi gli obiettivi di qualità perseguiti.
			...
			\\Gli obiettivi di qualità stabiliti per questo processo sono i seguenti:
				...
				Ogni attività di un processo verrà svolta da parte di colui a cui è stata assegnata, rispettando la pianificazione temporale stabilita nel documento \PdP{} e svolgendo tutti i compiti in cui essa si articola.\\(Metrica: {\hyperlink{sv}{schedule variance}}) 
				\subparagraph{Rispetto del budget}   
				I costi di ogni processo dovranno rientrare nel budget previsto dal documento \PdP.
				\\(Metrica: {\hyperlink{bv}{budget variance}})
			...
			\\Gli obiettivi di qualità stabiliti per questo processo sono i seguenti:
				...
				\\(Metrica: {\hyperlink{pc}{produttività di codifica}}) 
			...
			\\Gli obiettivi di qualità stabiliti per questo processo sono i seguenti:
				...
				Le funzionalità richieste sono state integrate all'interno di un prodotto software le cui diverse componenti interagiscono fra esse in modo coerente e senza criticità, a formare un sistema coeso e funzionante.\\(Metriche:\hyperlink{tue}{Test di unità eseguiti}, \hyperlink{tie}{Test di integrazione eseguiti}, \hyperlink{tse}{Test di sistema eseguiti}, \hyperlink{tae}{Test di accettazione eseguiti})
			...
			\\Gli obiettivi di qualità stabiliti per questo processo sono i seguenti:
				...
				La documentazione prodotta sarà quanto più chiara e comprensibile a tutti gli \gloss{stakeholder} coinvolti nel progetto.\\(Metrica: \hyperlink{ig}{Indice Gulpease})
			...
			\\Gli obiettivi di qualità stabiliti per questo processo sono i seguenti:
				...
				I test effettuati sulle diverse componenti del prodotto software andranno a considerare una gran parte delle possibili casistiche d'utilizzo e ad esplorare una notevole fetta del codice sorgente ad esso relativo.\\(Metrica: \hyperlink{cc}{Code Coverage})
...
\section{La strategia di gestione della qualità nel dettaglio}
	\subsection{Risorse}
		...
		\begin{itemize}
			...
			\item \textbf{Risorse software}
			Le risorse software disponibili sono costituite dagli \gloss{editor} di testo utilizzati per la stesura dei documenti in \gloss{Latex}, gli \gloss{IDE} impiegati a supporto dell'attività di codifica, le applicazioni online e le estensioni per i controlli sulla leggibilità e la complessità dei documenti in riferimento all'indice \gloss{Gulpease}. Più in dettaglio gli strumenti software impiegati per l'attività di Verifica sono illustrati nel documento \NdP.
			\end{itemize}
	\subsection{Misure e metriche \label{S5}}
		...
		\subsubsection{Pianificazione di progetto e processo di verifica e controllo}
			\paragraph[Schedule variance]{\hypertarget{sv}{Schedule variance}}
...
reale di fine di quell’attività: un valore di \textit{schedule variance} minore di zero indica che l'attività di progetto considerata sta richiedendo più tempo di quanto pianificato; se pari a zero indica invece che essa è perfettamente in linea con le tempistiche previste mentre se è addirittura strettamente positivo indica che sta procedendo più speditamente di quanto considerato. 
			...
			\begin{itemize}
				...
				\subsection{Processo di test per la qualifica del software/sistema}
				\label{P3}
			...
			\paragraph[Test di integrazione eseguiti]{\hypertarget{tie}{Test di integrazione eseguiti}}
			...
			\subsection{Processo di gestione della documentazione del software}
			\label{P4}
	...
	\subsection{Metriche per il prodotto}
		...
		Per quanto riguarda l'aspetto funzionale del software, si è scelto di seguire determinate metriche riguardanti il rispetto dei requisiti di maggior importanza, ossia quelli obbligatori e desiderabili.
			...
			\paragraph[Copertura requisiti desiderabili]{\hypertarget{crd}{Copertura requisiti desiderabili}}
		...
		Il gruppo \hx{} ha deciso di seguire metriche relative all'efficienza del software riguardanti taluni vincoli che il codice deve rispettare.
			\paragraph[Profondità di annidamento]{\hypertarget{pa}{Profondità di annidamento}}
		...
		Per garantire un elevato grado di manutenibilità, si è deciso di adottare un discreto numero di metriche che spaziano su diversi aspetti del programma.
			...
			\paragraph[Numero di campi dati per classe]{\hypertarget{ncdc}{Numero di campi dati per classe}}
			...
			Tale metrica è una misura del numero di dipendenze di una classe con le altre contenute all'interno del suo stesso \gloss{package}. Avere poche dipendenze tra classi implica che ci sia un maggiore grado di disaccoppiamento. Questo aumenta molto la manutenibilità e la comprensibilità del codice.
			\\I range stabiliti per la metrica sono i seguenti:
			\begin{itemize}
			...
			\paragraph[Complessità ciclomatica]{\hypertarget{cci}{Complessità ciclomatica}}
			...
			\paragraph[Fan-in]{\hypertarget{fi}{Fan-in}}
			...
			\paragraph[Validazione W3C]{\hypertarget{vw}{Validazione W3C}}
	...
	\hline
	\multicolumn{3}{|l|}{\textbf{Metriche di Processo}}\\% & \multicolumn{2}{|l|}{}
	\hline
	...
	\hline
	\multicolumn{3}{|l|}{\textbf{Metriche di prodotto}}\\% & \multicolumn{2}{|l|}{}
	\hline
...
\section{Test}
	...
	\begin{itemize}
		...
		\item \textbf{Test di integrazione [TI]}: test per verificare le componenti di sistema con l'obiettivo di verificare il funzionamento dei vari \gloss{package} prodotti, singolarmente o nel loro insieme;
