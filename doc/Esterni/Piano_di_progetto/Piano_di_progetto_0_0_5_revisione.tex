...
\section{Introduzione}
	...
	{\color{blue}\\Come detto da Alberto nelle norme di progetto(e guardando gli altri gruppi) sarebbe da spostare la struttura del documento da uqalche altra parte e piuttosto fare una descrizione puntata dello scopo di questo docuento del tipo:}
	\\In particolare, questo documento si appresta a:
	\begin{itemize}
		\item analizzare e trattare i rischi;
		\item preventivare le risorse necessarie;
		\item pianificare le atticità da svolgere;
		\item fornire un consuntivo del lavoro svolto.
	\end{itemize}
	{\color{blue}fine proposte}
...
{\color{blue} ok non c'è su quello degli altri gruppi sta roba. Io l'ho letta ma mi fido e basta di quello che è effettivamente spiegato}
...
{\color{blue} OK perdonatemi perché rompo le scatole pure io alcune volte...ma preferiscono fare un misto tra tipo leaf e thefellowship per le parole di alcune frasi...ah e penso che metterò i due punti agli elenchi}
...
\begin{description}
	\item[Identificazione dei rischi]: individuare i rischi potenziali che si possono presentare durante lo sviluppo del progetto e capirne le cause. Si possono identificare tre diverse tipologie di rischio:
	\begin{description}
...
\end{description}
Ogni rischio è caratterizzato da un elenco di informazioni che comprende:
\begin{description}
	\item[Nome];
	\item[Descrizione];
	\item[Probabilità di occorrenza];
	\item[Livello di pericolosità];
	\item[Riconoscimento];
	\item[Trattamento];
...
\end{description}
	\subsection{Livello tecnologico} \label{sec:tec}
		...
		\begin{description}
			...
			\item[Livello di pericolosità]: alto;
		...
		\begin{description}
			\item[Descrizione]: la strumentazione hardware e software usata dal gruppo può essere soggetta a rotture e malfunzionamenti durante lo svogimento del progetto. Un altro rischio di fallibilità hardware è quello del \gloss{server} usato per ospitare 				\gloss{PragmaDB}, un malfunzionamento su tale macchina o sui pc dei membri del gruppo mette a rischio il lavoro dell'intero \gloss{team}, rendendo così più difficile l'avanzamento;
			\item[Probabilità di occorrenza]: bassa;
			\item[Livello di pericolosità]: medio;
			...
			{\color{blue} non mi piace la frase scaricare sul server github. Direi piuttosto sincronizzare o caricare più che scaricare. Al massimo se avete altre idee al momento sistemate voi}
			\item[Trattamento]: prima di qualunque spostamento, è obbligatorio sincronizzare sul \gloss{server} \gloss{GitHub} i file modificati. Se ciò non fosse possibile per assenza di collegamento ad Internet, è obbligatorio copiare i propri avanzamenti su almeno un dispositivo di memoria esterno secondario. Per il \gloss{server} che ospita \gloss{PragmaDB}, è previsto un sistema di \gloss{backup} automatico ogni 7 giorni e in caso di malfunzionamenti sarà compito dell’\AM{} riportare tale macchina in uno stato funzionante nel minor tempo possibile; 
			\item[Attuazione nel periodo]:
			\begin{description}
	...
	\subsection{Livello del personale} \label{sec:per}
		...
		\begin{description}
			...
			\item[Probabilità di occorrenza]: media;
			\item[Livello di pericolosità]: medio;
			\item[Riconoscimento]: ognuno è tenuto a comunicare tempestivamente i propri impegni al \R{} in modo da create un calendario sincronizzato e condiviso del gruppo;
	...
	\subsection{Livello della valutazione dei costi} \label{sec:val}
		...
		\begin{description}
			...
			\end{description}
		\end{description}
		{\color{blue} fine verifica per ora}
...
\begin{table}[htbp]
	\begin{center}
		...
		\begin{tabular}{|c|c|p{5cm}|}
			...
			\hline
			\LB & 25-12-2016 %& \myincludegraphics{path firma}
			 \\[1ex]
			\hline
		\end{tabular}
	\end{center}
...
\begin{table}[htbp]
	\begin{center}
		...
		\begin{tabular}{|c|c|p{5cm}|}
			...
			\hline
			\LB		& 24-01-2017					%& \myincludegraphics{path firma}			
			\\[1ex]
			\hline
			\TV	&								&			\\[1ex]
			\hline
...
\begin{table}[htbp]
	\begin{center}
		...
		\begin{tabular}{|c|p{3.5cm}|p{6cm}|}
			...
			\hline
			\AZ	&	24-01-2017	%& \myincludegraphics{path firma}	
			\\[1ex]
			\hline
			\GG		&	24-01-2017	%& \myincludegraphics{path firma}		
			\\[1ex]
			\hline
			\LB		&	24-01-2017	%& \myincludegraphics{path firma}		
			\\[1ex]
			\hline
			\LS		&	24-01-2017	%& \myincludegraphics{path firma}		
			\\[1ex]
			\hline
			\MM		&	24-01-2017	%& \myincludegraphics{path firma}		
			\\[1ex]
			\hline
			\PB		&	24-01-2017	%& \myincludegraphics{path firma}		
			\\[1ex]
			\hline
		\end{tabular}
	\end{center}
