...
\section{Introduzione}
	...
	{\color{blue}\\Credo sia meglio fare una descrizione puntata dello scopo di questo documento e separare la struttura del documento del tipo:
...
{\color{blue} OK sezione non presente dagli altri la grammatica mi sembra ok ma di quello che cè scritto mi fido\\}
\section{Modello di sviluppo} \label{sec:modello}
	...
	Il progetto {\proj}, come affrontato nei documenti \emph{Studio di fattibilità} e \emph{Piano di Qualifica}, presenta numerosi punti critici, analizzati nella sezione {\ref{sec:analisi}}, in particolare nella sezione {\ref{sec:tec}}, in cui sono analizzati i rischi tecnologici. Generalmente {\proj} richiede una intensiva fase di progettazione, il cui fine ultimo è l'esplorazione delle problematiche legate ai software CASE e alla loro risoluzione, tenendo conto del quantitativo di risorse assegnate al progetto. %[riferimento a verbale di riunione con zucchetti?]
...
Il primo consiste nel lavorare a fianco del \gloss{client}e per esplorare i suoi requisiti e infine consegnare un sistema adeguato. Mentre questo metodo di lavoro potrebbe sembrare positivo, esso presenta due grosse criticità: 
\begin{itemize}
...
\end{itemize}
Questo rischio a parere del team non riesce ad essere bilanciato dalla possibilità di ritardare requisiti e decisioni di design.
...
In complesso, il team ritiene che il modello incrementale apporti sostanziali vantaggi rispetto agli altri modelli finora analizzati. In seguito si analizzano altri modelli che sono stati presi in considerazione.
...
{\color{blue} OK perdonatemi perché rompo le scatole pure io alcune volte...ma preferiscono fare un misto tra tipo leaf e thefellowship per le parole di alcune frasi...segno in blu le robe nuove e in rosso quelle tolte\\}
{\color{blue}La seguente sezione elenca e descrive i possibili rischi che possono andare ad inficiare sul lavoro del gruppo.
La gestione dei rischi è divisa in cinque sottofasi:\\}
{\color{red}In questa sezione del documento vengono elencati e descritti tutti i possibili rischi che potrebbero colpire il gruppo \hx{} nella realizzazione del prodotto \proj. Per gestire tali rischi, è stata attuata la seguente procedura che prevede:}
\begin{description}
	\item[Identificazione dei rischi] {\color{blue}individuare i rischi potenziali che si possono presentare durante lo sviluppo del progetto e capirne le cause. Si possono identificare tre diverse tipologie di rischio:}{\color{red}trovare i rischi potenziali che si possono presentare durante l'intero sviluppo del progetto e studiarne la natura. Essi possono essere di tre tipologie:}
	{\color{blue} itemize e due punti qua sotto per un pò di roba}
	\begin{itemize}
	...
	\end{itemize}
	\item[Analisi dei rischi]{\color{blue} per ogni rischio si deve stimare le:}{\color{red} studiare per ogni rischio le:}
	\begin{itemize}
		\item[Probabilità di occorrenza];
		\item[Conseguenze]:{\color{blue} determinarne l'impatto sul progetto per comprenderne le criticità.}{\color{red} comprendere che peso hanno sul progetto per comprenderne le criticità.}
	\end{itemize}
	\item[Pianificazione di controllo e mitigazione]{\color{blue}definire metodi di controllo per i rischi, in modo da poterli evitare tramite:} {\color{red}istituire metodi di controllo per i rischi, così da poterli evitare facendo:}
	\begin{itemize}
		\item[Verifica costante del livello di rischio];
		\item[Riconoscimento e trattamento].
	\end{itemize}
	{\color{blue}\item[Mitigazione] minimizzare i danni nel caso si fossero verificati tramite:
	\begin{itemize}
...
\end{description}
{\color{blue}Ogni rischio è caratterizzato da un elenco di informazioni che comprende:}{\color{red}
...
\end{itemize}
	\subsection{Livello tecnologico} \label{sec:tec}
		...
		\begin{description}
			...
			\item[Trattamento] prima di qualunque spostamento, è obbligatorio {\color{blue}sincronizzare}{\color{red}scaricare} sul server \gloss{Git}Hub i file modificati. Se ciò non fosse possibile per assenza di collegamento ad Internet, è obbligatorio copiare i propri avanzamenti su almeno un dispositivo di memoria esterno secondario. Per il server che ospita PragmaDB, è previsto un sistema di \gloss{backup} automatico ogni 7 giorni e in caso di malfunzionamenti sarà compito dell’\AM{} riportare tale macchina in uno stato funzionante nel minor tempo possibile; 
			\item[Attuazione nel periodo]:
			\begin{description}
	...
	\subsection{Livello del personale} \label{sec:per}
		...
		\begin{description}
			\item[Descrizione] i componenti del gruppo sono alle prime esperienze nello sviluppo di progetti dove il numero di partecipanti è alto. {\color{blue} Questo potrebbe portare ad incomprensioni e dissidi tra i membri del gruppo, generando quindi un clima non proficuo;}{\color{red}Tale fattore potrebbe causare problemi quali incomprensioni tra i membri del gruppo e dissidi, generando quindi un clima non proficuo;}
			\item[Probabilità di occorrenza] bassa;
			\item[Grado di pericolosità] alto;
		...
		\begin{description}
			...
			\item[Probabilità di occorrenza] media;
			\item[Grado di pericolosità] medio;
			\item[Riconoscimento] {\color{blue}ognuno è tenuto a comunicare tempestivamente i propri impegni al \Rx{} in modo da create un calendario sincronizzato e condiviso del gruppo;}{\color{red}per creare un calendario sincronizzato e condiviso del gruppo, è necessario che vengano notificati al \Rx{} in maniera preventiva e tempestiva gli impegni di ognuno;}
			...
			\item[Attuazione nel periodo]:
			\begin{description}
				\item[Analisi dei requisiti]{\color{blue}il rischio si è verificato  a due membri del gruppo che hanno dovuto abbandonare la riunione prima della conclusione per problemi personali;} {\color{red}il rischio si è verificato un paio di volte a due membri del gruppo per problemi personali; nonostante ciò, il lavoro è stato portato al termine e non ci sono stati rallentamenti.}
			\end{description}
		\end{description}
		...
		\begin{description}
			...
			\item[Grado di pericolosità] alto;
			\item[Riconoscimento] {\color{blue} È compito del \Rx{} verificare il livello di conoscenza del gruppo riguardo le tecnologie necessarie allo svoglimento del progetto} {\color{red}quando un componente del gruppo ritiene opportuno utilizzare una nuova tecnologia, deve segnalarlo al \Rx{} che, una volta approvatone l'utilizzo nel progetto, demanderà al gruppo il compito di documentarsi su come 	impiegarlo al meglio;}
			\item[Trattamento]{\color{blue} ogni membro del gruppo ha il compito di documentarsi su strumenti e tecnologie utili allo svolgimento del progetto. Nel caso in cui questo non fosse sufficiente, il \Rx{} dovrà preparare un piano di studi per compensare ogni tipo di lacuna;}
			\item[Attuazione nel periodo]:
			\begin{description}
...
Per eseguire una più accurata pianificazione progettuale, il team ha deciso di suddividere il progetto nelle seguenti fasi:
\begin{description}
...
\label{tab:genweeks}
\caption{Diagramma di Gantt della pianificazione generale, in settimane (parte 1)}
...
Di seguito è riportata la pianificazione di dettaglio delle attività, suddivisa nei periodi descritti precedentemente. Per ogni periodo, sono presenti una tabella e dei grafi che descrivono e rappresentano i ruoli assunti da ciascun componente, il relativo quantitativo di ore impiegato e la rispettiva retribuzione.
...
%\rowcolors{1}{white}{white2}
  ...
  \begin{tabular}{ | l | c | c | c | c | c | c | r |}
    ...
    \MM & 0 & 0 & 0 & 990 & 0 & 60 & 1050 \\ \hline
    \LS & 60 & 0 & 0 & 880 & 0 & 90 & 1030 \\ \hline
    \AZ & 0 & 0 & 0 & 792 & 0 & 60 & 852 \\ \hline
    \rowcolor[gray]{.9}
...
\makebox[\textwidth][c]
{
  \begin{tabular}[width=1.2\textwidth]{ | l | c | c | c | c | c | c | r |}
    ...
    Totale ore/ruolo & 15 & 29 & 15 & 280 & 180 & 115 & 634 \\ \hline
...
\label{tab:cgen1}
  ...
  \caption{Variazione rispetto alla pianificazione per fase di \AR, \gloss{diagramma di Gantt} (parte 1)}
\end{figure}
...
Questo ha portato un ritardo complessivo pari a 3 giorni rispetto alla baseline; tuttavia il task, non presentando dipendenze, non ha portato ritardi se non all'attività di verifica relativa al documento stesso.
