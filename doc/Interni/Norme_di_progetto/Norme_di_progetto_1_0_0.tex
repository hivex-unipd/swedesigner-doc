...
\subsection{Sviluppo}
	...
	% \begin{itemize}
		...
		% \item \gloss{Diagramma dei package}: raggruppa un numero di elementi UML in una sola unità di livello più alto.
		% \item \gloss{Diagramma di attività}: descrive in dettaglio un algoritmo
		% \item \gloss{Diagramma di sequenza}: descrive uno scenario, dove le azioni sono disposte in sequenza e le varie scelte sono già state prese.
	% \end{itemize}
	...
	\subsubsection{Strumenti}
		...
		\paragraph{Astah}
		L'editor \gloss{UML} che gli analisti dovranno usare è \gloss{Astah}, creato da \emph{ChangeVision} e disponibile gratuitamente online. Supporta la generazione di tutti i diagrammi che vengono usati dal gruppo (classi, attività, sequenza e package).
...
\end{itemize}
\paragraph{Formato} Ogni documento dev'essere in formato \gloss{Latex} con estensione \texttt{.tex} (esportabile in formato PDF su foglio A4); la prima versione di un documento può essere in altri formati ma va presto sostituita da una versione in \gloss{Latex}. I file \gloss{Latex} che definiscano delle \gloss{macro} o diano istruzioni di stile iniziano con la sigla "hx-" (per rimarcare che sono librerie create dal gruppo), hanno estensione \texttt{.sty} (tranne "hx-ambiente", per permetterne l'inclusione prima di dichiarare la classe del documento) e vanno inclusi con il comando \texttt{\textbackslash usepackage\{\}}.
...
\paragraph{Branching} Nel caso si debba cambiare un elemento della configurazione da cui dipendono altri elementi --- modificando quindi gli elementi dipendenti --- il membro del gruppo che è stato incaricato di tale cambiamento deve creare un \gloss{branch} nel repository. Un \gloss{branch} (ramo) sottosta alle seguenti regole:
\begin{itemize}
...
\end{enumerate}
	...
	\begin{description} %%% DA RIEMPIRE MAN MANO
		...
		\item[sintassi \gloss{Latex}] il documento deve usare le macroistruzioni definite nei file della directory \texttt{util} del ramo \texttt{doc} del nostro gloss{repository};
...
\subsubsection{Riunioni}
	...
	\paragraph{Verbali}
		...
		\end{itemize}
		Segue, infine, un riassunto esaustivo riguardo le decisione prese, eventuali dubbi o problemi ed eventuali compiti assegnati.
\subsubsection{Comunicazioni esterne}
	Per le comunicazioni esterne il gruppo ha creato un indirizzo di posta elettronica (\email). È compito del responsabile di progetto comunicare con il committente e l'università e informare i restanti membri di cosa si è discusso. % magari si può anche dire che abbiamo l'inoltro automatico per infomare gli altri.
...
\paragraph{Baseline} Infine è compito del Responsabile di Progetto salvare una \emph{baseline} del progetto ad ogni milestone raggiunta: in questo modo è possibile confrontare il lavoro pianificato con quello effettivamente svolto e stilare il consuntivo.
