...
\subsection{Sviluppo}
	...
	% La fase di progettazione deve utilizzare come diagrammi \gloss{UML} quelli sotto descritti,in riferimento allo standard \gloss{UML} 2.0:
	% \begin{itemize}
		% \item \gloss{Diagramma delle classi}: descrive i tipi di entità(classi) e le relazioni tra loro.
		% \item \gloss{Diagramma dei package}: raggruppa un numero di elementi \gloss{UML} in una sola unità di livello più alto.
		% \item \gloss{Diagramma di attività}: descrive in dettaglio un algoritmo
		% \item \gloss{Diagramma di sequenza}: descrive uno \gloss{scenario}, dove le azioni sono disposte in sequenza e le varie scelte sono già state prese.
	% \end{itemize}
	...
	\subsubsection{Strumenti}
		\paragraph{PragmaDB}
		Per tracciare le relazioni tra casi d'uso e requisiti, il gruppo ha scelto di usare \gloss{PragmaDB} (\url{https://github.com/StefanoMunari/PragmaDB}), software realizzato da studenti degli anni passati. %Gli analisti possono accedervi via browser collegandosi al \gloss{server} creato dal gruppo all'indirizzo \url{http://ec2-54-93-172-72.eu-central-1.compute.amazonaws.com} tramite autenticazione.
		
		Esso consente la gestione automatizzata della gerarchia dei casi d'uso, dei requisiti e delle voci del glossario. Risulta quindi un supporto agevole per rendere il lavoro di gruppo in fase di analisi più efficiente, uniforme e coeso. Inoltre permette l'esportazione in \gloss{Latex} delle varie tabelle (requisiti, casi d'uso, glossario\dots) seguendo le linee guida di struttura descritte in questo documento.
		\paragraph{Astah}
		L'\gloss{editor} \gloss{UML} che gli analisti dovranno usare è \gloss{Astah}, creato da \emph{ChangeVision} e disponibile gratuitamente online. Supporta la generazione di tutti i diagrammi che vengono usati dal gruppo (classi, attività, sequenza e \gloss{package}).
...
La \gloss{documentazione} di progetto va sviluppata nel \gloss{branch} \texttt{doc} del nostro \gloss{repository}, come motivato nella sezione \ref{sec:config}.
...
\paragraph{Nome} Il nome di un file o di una directory non dovrebbe contenere spazi, al fine di facilitare operazioni da riga di comando. Inoltre deve indicare in modo quanto più chiaro e non ambiguo il contenuto del documento stesso. In particolare:
\begin{itemize}
	...
	\item I file \gloss{Latex} che definiscano delle macro-istruzioni o diano istruzioni di stile iniziano con la sigla "hx-" (per rimarcare che sono librerie create dal gruppo).
\end{itemize}
\paragraph{Formato} Ogni documento dev'essere in formato \gloss{Latex} con estensione \texttt{.tex} (esportabile in formato \gloss{PDF} su foglio A4); la prima \gloss{versione} di un documento può essere in altri formati ma va presto sostituita da una versione in \gloss{Latex}. I file \gloss{Latex} che definiscano delle \gloss{macro} o diano istruzioni di stile iniziano con la sigla "hx-" (per rimarcare che sono librerie create dal gruppo), hanno estensione \texttt{.sty} (tranne "hx-ambiente", per permetterne l'inclusione prima di dichiarare la classe del documento) e vanno inclusi con il comando \texttt{\textbackslash usepackage\{\}}.
...
\subsubsection{Macro-istruzioni \gloss{Latex}}
Per uniformare il testo, i redattori sono tenuti a usare le seguenti macro-istruzioni \gloss{Latex}, definite nelle librerie della directory \texttt{util}:
\begin{itemize}
	\item \texttt{\textbackslash proj}, il nome del progetto;
	\item \texttt{\textbackslash hx}, il nome del gruppo;
	\item \texttt{\textbackslash repo}, l'indirizzo web del \gloss{repository};
...
Definiamo la configurazione di un prodotto come l'insieme delle parti di cui si compone e il modo in cui esse interagiscono fra di loro. Per gestire la configurazione del software va utilizzato il sistema \gloss{Git}, appoggiandosi alla piattaforma \gloss{GitHub}. Abbiamo scelto \gloss{GitHub} non solo per la qualità del servizio fornito ma anche per la natura open-source del progetto \proj, che ci abilita ad usare questa piattaforma gratuitamente. Il \gloss{repository} creato all'indirizzo \repo{} si divide nelle seguenti directory:
\begin{description}
...
\paragraph{Branching} Nel caso si debba cambiare un elemento della configurazione da cui dipendono altri elementi --- modificando quindi gli elementi dipendenti --- il membro del gruppo che è stato incaricato di tale cambiamento deve creare un \gloss{branch} nel \gloss{repository}. Un \gloss{branch} (ramo) sottosta alle seguenti regole:
\begin{itemize}
	...
	\item Il numero di rami va contenuto, affinché \gloss{Git} rimanga uno strumento e non diventi un peso per i membri del gruppo: la creazione di un nuovo ramo può avvenire solo previa autorizzazione del responsabile di progetto, che dovrà valutarne l'utilità.
...
Nel ramo \texttt{master} del \gloss{repository} va pubblicato lo stato attuale della configurazione, cioè va riportato il numero di versione dell'ultima approvazione formale dell'\emph{intero} software, a partire dalla versione zero (non ancora approvata).
...
\end{enumerate}
	...
	\begin{description} %%% DA RIEMPIRE MAN MANO
		...
		\item[sintassi \gloss{Latex}] il documento deve usare le macro-istruzioni definite nei file della directory \texttt{util} del ramo \texttt{doc} del nostro \gloss{repository};
...
\begin{itemize}
	...
	\item la pubblicazione dei componenti validati e dei test eseguiti sul ramo \texttt{master} del \gloss{repository}.
\end{itemize}
...
\begin{itemize}
	\item principalmente, la \textbf{mancanza di struttura} che dovrebbe permettere a \gloss{Trello} un lavoro più agile si è rivelata inefficace in fase di pianificazione;
	\item lo strumento EleGantt, usato per stendere dei \gloss{diagrammi di Gantt} (non nativi in \gloss{Trello}) era troppo limitato nella sua versione gratuita.
\end{itemize}
Il gruppo ha quindi adottato i servizi offerti da \gloss{Asana}, accoppiato a \gloss{InstaGantt} (un servizio esterno molto ben integrato con \gloss{Asana}), principalmente per avvalersi delle seguenti funzionalità:
\begin{itemize}
...
\subsubsection{Riunioni}
		...
		\subsubsection{comunicazioni esterne}
			...
			È compito del responsabile di progetto comunicare con il committente e l'università e informare i restanti membri di cosa si è discusso. %magari si può anche dire che abbiamo l'inoltro automatico per infomare gli altri.
...
\begin{itemize}
	...
	\item integrazione con servizi esterni (ad esempio con \gloss{GitHub}).
\end{itemize}
...
\begin{description}
	\item[\#documents] come strutturare i documenti e problematiche relative all'uso di \gloss{Latex};
	...
	\item[\#github] problematiche nell'uso di \gloss{Git}, integrazione con \gloss{GitHub} (vedi prossimo paragrafo);
	\item[\#meta] strutturazione canali di \gloss{Slack};
...
\paragraph{Integrazione con GitHub} \gloss{Slack} offre, nella sua versione gratuita, l'integrazione di un massimo di 10 servizi esterni. Un servizio che si è rivelato utile è stato l'integrazione con \gloss{GitHub}: questa crea un bot, inserito nel canale \#github, il quale invia notifiche relative a \gloss{commit}, \gloss{pull request} e attività nei \gloss{GitHub Issues}.
...
\begin{description}
	...
	\item[data di inizio prevista] (da \gloss{InstaGantt})
	...
	\item[dipendenze tra task] (da \gloss{InstaGantt}) ogni task può necessitare il completamento di un altro task per poterlo iniziare;
	\item[ore previste] (da \gloss{InstaGantt})
	\item[ore effettive] (da \gloss{InstaGantt})
...
\begin{description}
	\item[Milestone] contiene le \gloss{milestone} decise dal gruppo e quelle obbligatorie, definite dal \TV;
...
\gloss{InstaGantt} permette di visualizzare i vari task in un asse temporale, producendo così un \gloss{diagramma di Gantt}. 
...
\end{itemize}
La sezione \emph{Workload} in \gloss{InstaGantt} permette di visualizzare i vari task assegnati ad ogni persona; grazie a questo si può analizzare il quantitativo di ore di lavoro impiegato giornalmente, con la possibilità di filtrarlo per ruolo, attraverso l'uso dei tag. Per questo, il responsabile di progetto è tenuto a:
\begin{itemize}
...
\paragraph{Baseline} Infine è compito del responsabile di progetto salvare una \emph{baseline} del progetto ad ogni \gloss{milestone} raggiunta: in questo modo è possibile confrontare il lavoro pianificato con quello effettivamente svolto e stilare il consuntivo.
