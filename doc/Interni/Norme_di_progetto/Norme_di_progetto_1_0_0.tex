...
\subsection{Sviluppo}
	...
	Il team seguirà due diverse convenzioni in base al linguaggio utilizzato. Essendo la parte client scritta in javascript verranno seguite le indicazioni riguardo nomenclatura, regole sull'uso di spazi, indentazioni e commenti descritti su \url{http://javascript.crockford.com/code.html}. La parte server è scritta in java e seguirà le indicazioni di \url{http://www.oracle.com/technetwork/java/javase/documentation/codeconvtoc-136057.html}.
	\paragraph{Nomi}
	Sia per la parte client che per la parte server i la nomenclatura di classi, metodi e variabili segue la notazione a cammello, con alcune specifiche:
	\begin{itemize}
		\item variabili e metodi di Javascript iniziano con la lettera minuscola;
		\item le classi di Java usano la lettera iniziale maiuscola;
		\item variabili e metodi di Java usano la lettera iniziale minuscola.
	\end{itemize}
	\paragraph{Ricorsione}
	Bisogna sempre evitare di ricorrere alla ricorsione. Nel caso non fosse possibile trovare un metodo alternativo iterativo è obbligatorio fornire una prova della sua terminazione. 
	\paragraph{Commenti}
	Per fornire un'adeguata documentazione al progetto è necessario fare uso di commenti nel codice. In particolare oltre a seguire le indicazioni precisate nella sezione \ref{sec:conv} riguardo lo stile di essi, si integrano inoltre commenti per la generazione di documentazione tramite javadoc.
	\begin{itemize}
	\item all'inizio di ogni file .java si inserisce un commento riguardante il documento con la seguente struttura:
	\begin{equation}
	/**
	...
	*/
	\end{equation}
	\item all'inizio di ogni metodo si inserisce un commento con la struttura descritta in seguito:
	\begin{equation}
	 /** 
     ...
     */
	\end{equation}
	Il numero di @param dipende dal numero di parametri del metodo.	@return si omette nel caso di metodi void e costruttori. Alcuni parametri aggiuntivi posso essere \emph{@throws TipoEccezione Motivo} nel caso di eccezioni controllate presenti nel metodo e \emph{@since} se il metodo è stato inserito in una versione diversa rispetto al file.
...
Da sottolineare che l'utilizzo di periodi di \gloss{slack} aumenta la possibilità che la \emph{schedule variance} assuma valori positivi.
...
La metrica indica la percentuale di istruzioni che sono eseguite durante i test. Un'alta percentuale di istruzioni coperte dai test eseguiti comporta una maggiore probabilità che le componenti testate abbiano un ridotto numero di errori. Si può aumentare tale indice evitando di considerare i metodi che non richiedono test, come i metodi \gloss{getter} e \gloss{setter}.
