...
\subsection{Sviluppo}
	...
	% \begin{itemize}
		% \item Diagramma delle classi: descrive i tipi di entità(classi) e le relazioni tra loro.
		% \item Diagramma dei \gloss{package}: raggruppa un numero di elementi UML in una sola unità di livello più alto.
		% \item Diagramma delle attività: descrive in dettaglio un algoritmo
	...
	\subsubsection{Strumenti}
		...
		Per tracciare le relazioni tra casi d'uso e requisiti, il gruppo ha scelto di usare PragmaDB (\url{https://github.com/StefanoMunari/PragmaDB}), software realizzato da studenti degli anni passati. %Gli analisti possono accedervi via browser collegandosi al \gloss{server} creato dal gruppo all'indirizzo \url{http://ec2-54-93-172-72.eu-central-1.compute.amazonaws.com} tramite autenticazione.
...
\begin{itemize}
	...
	\item I file Latex che definiscano delle macro-istruzioni o diano istruzioni di stile iniziano con la sigla "hx-" (per rimarcare che sono librerie create dal gruppo).
\end{itemize}
...
\begin{itemize}
	...
	\item riferimento al repository pubblico di \gloss{Git}Hub.
\end{itemize} 
...
\subsubsection{Termini del glossario} All'interno di un documento, la prima occorrenza di un termine che si trovi nel glossario dev'essere evidenziata e segnata con il pedice “\ped{$g$}” tramite la macro \texttt{\textbackslash gloss\{termine\}}, per indicare al lettore di ricercarne il significato nel glossario.
...
\begin{itemize}
	...
	\item Le parole straniere di uso comune (computer, radar, web\dots) non vanno evidenziate, per non appesantire il testo; vanno invece evidenziate quelle straniere di uso \emph{non} comune, come \emph{\gloss{framework}} o \emph{way of working}. Quando un termine straniero ha una traduzione italiana che non sia troppo strana o artificiosa, va preferita la traduzione in italiano.
...
Definiamo la configurazione di un prodotto come l'insieme delle parti di cui si compone e il modo in cui esse interagiscono fra di loro. Per gestire la configurazione del software va utilizzato il sistema Git, appoggiandosi alla piattaforma \gloss{GitHub}. Abbiamo scelto GitHub non solo per la qualità del servizio fornito ma anche per la natura open-source del progetto \proj, che ci abilita ad usare questa piattaforma gratuitamente. Il repository creato all'indirizzo \repo{} si divide nelle seguenti directory:
\begin{description}
