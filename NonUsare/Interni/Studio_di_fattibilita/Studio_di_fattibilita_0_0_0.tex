...
\section{Introduzione}
	...
	\subsection{Riferimenti}
		...
		\begin{itemize}
			...
			\item \textbf{Capitolato d'Appalto C6:} \emph{SWEDesigner}: editor di diagrammi \gloss{UML} con generazione di codice
			\\ \url{http://www.math.unipd.it/~tullio/IS-1/2016/Progetto/C6.pdf};
		\end{itemize}
\newpage
	
\section{Studio di fattibilità del capitolato scelto}
	\subsection{\emph{SWEDesigner}: editor di diagrammi \gloss{UML} con generazione di codice}
		\subsubsection{Descrizione}
		Lo scopo del capitolato è quello di realizzare un'\gloss{applicazione web} composta da almeno due disegnatori
		...
		\subsubsection{Studio del dominio}
			...
			\\In particolare lo scopo profondo del capitolato è quello di analizzare quali diagrammi \gloss{UML} siano i più adatti allo scopo di descrivere l'architettura 
			...
			Il capitolato richiede che il sistema sia realizzato con tecnologie Web. In particolare la parte server deve essere realizzata in \gloss{Java} con il server 
			Tomcat o in \gloss{Javascript} con il server Node.Js. La parte client, invece, deve essere eseguibile in un browser \gloss{HTML5} ed utilizzare fogli di stile CSS per 
		...
		con problemi quali la generazione automatica di codice e il raffinamento dell'\gloss{UML}, ancora vivi nell'ambito informatico.
...
\section{Studio di fattibilità degli altri capitolati}
	\subsection{Capitolato C1: \emph{APIM}: An API Market Platform}
		\subsubsection{Descrizione}
		Lo scopo di questo capitolato è quello di creare un'\gloss{applicazione web} che rappresenti una sorta di marketplace per la consultazione e 
		...
		\subsubsection{Studio del dominio}
			...
			margine di libertà. Il front end della web application è da svilupparsi con l'ausilio della terna \gloss{JavaScript}, \gloss{HTML5} e \gloss{CSS3} mentre a 
	...
	\subsection{Capitolato C2: \emph{AtAVi}: Accoglienza tramite Assistente Virtuale}
		...
		sfrutti i servizi \gloss{AWS} Lambda per l'interazione con le API dell'assistente virtuale scelto.
		\subsubsection{Studio del dominio}
			\paragraph{Dominio applicativo}
			...
			\begin{itemize}
				...
				di \gloss{AWS} che consentono di implementare un servizio di elaborazione servless;
		...
		lavorare con il supporto di un server dedicato configurato tramite i servizi \gloss{AWS}.\\
	...
	\subsection{Capitolato C3: \emph{DeGeOP}: A Designer and Geo-localizer Web App for Organizational Plants}
		...
		\subsubsection{Studio del dominio}
			...
			\begin{itemize}
				\item \gloss{AWS}, come servizio per l'organizzazione di un server entro cui far operare l'applicativo;
				...
				\item HTML, CSS e \gloss{JavaScript} per la realizzazione dell'interfaccia utente.
			\end{itemize}
		\subsubsection{Aspetti positivi e negativi}
		\subsubsection{Valutazione finale}
