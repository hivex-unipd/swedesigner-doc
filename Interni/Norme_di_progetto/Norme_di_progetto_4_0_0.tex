...
Lo Studio di Fattibilità deve presentare l'analisi svolta dai membri del gruppo riguardo ad ogni capitolato d'appalto; per ogni capitolato, gli analisti devono inserire le seguenti sezioni:
\begin{description}
...
\subsubsection{Consegna del prodotto} \label{sec:supply}
...
\subsection{Validazione} \label{sec:valid}
Il gruppo deve accertare la correttezza e completezza del prodotto. Per questo, i membri del gruppo dovranno eseguire i test di validazione specificati nel Piano di Qualifica (versione attuale: \PdQ).

I verificatori devono eseguire i test prima del collaudo; inoltre, tutti i membri devono essere preparati al fatto che, durante il collaudo, il committente e/o il proponente richiedano l'esecuzione di uno qualsiasi tra i test di validazione specificati.

Il gruppo, infine, deve eseguire un collaudo pubblico alla presenza del committente e, se possibile, del proponente. Questo collaudo è normato alla sezione \ref{sec:supply}.
...
Pertanto se un valore troppo alto di tale metrica è indice di un grado di complessità dell'architettura da limitarsi, un valore eccessivamente basso sottolinea un potenziale sottoutilizzo dei vantaggi dell'architettura orientata agli oggetti.
